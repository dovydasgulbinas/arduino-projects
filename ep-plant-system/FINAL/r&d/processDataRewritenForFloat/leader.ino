...
float parseFloat(char terminator = '|');
...
void loop() {
  ...
  char serialData = parseFloat(':'); // dataRed is suitable for processing
  if (newData) {
    if (serialData == 1) {
      Serial.println("@1");//-------------
      float t = parseFloat();
...
float parseFloat(char terminator) //If more data is sent to the buffer code wont work//Will return a char array of bytes from serial port //x param+1
{

...
float refVoltage = 4.44;
float currentConstant = 0.1 ;// V/A
float currentVoltageConstant = refVoltage/(1024*currentConstant);
//boolean measureCurrent = true;
...
if((callCounter%callMeasurment)==0){
...
void setup()
{
	...
	powerStr[0] = {0,0,111,0,{0},currentPin_0,0};
	powerStr[1] = {0,0,222,0,{0},currentPin_1,0};
	powerStr[2] = {0,0,333,0,{0},currentPin_2,0};
	////	
...
void loop()
{
 ...
 if((millis()%250)==0){// Scanns button state every 0.1 sec // register changes
 Serial.print("RMS0: ");
  Serial.println(powerStr[0].RMSCurrent);
  //powerStr[1].power = analogRead(currentPin_1);
 // powerStr[2].power = analogRead(currentPin_2);

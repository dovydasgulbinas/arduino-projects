 /*#####################
A.[TEST] dimme-r functionality while using buttons
<PORTB pins 8-13>
  1.Rewire the dim light pin and relay pins
  2.Change timer constant to 75
  3.Comment out powerMeasuring Code
  4.Press buttons and see weird behavior buttonPin_2 should be defected
B.[WRITE] Measure how many microseconds ISR takes and by this number determine the number of brightness steps  
  1. After finding appropriate number of brightness steps write a code in which you implement Dimmer + Power measure together
  2. Conclude whether its possible to do in one ISR    B = 1/2f*t // t - time it takes for ISR to execute; B -number of brightness steps for a lightBulb; f - network frequency
  3. Take brightness error in to account while developing full code
  
 ####AFTERMATH################
1.Button code does not work if you use pins that belond to a separate register
2.ZERO CROSS IS NOT REGISTERED!
#######################*/
#include <Wire.h>
#include  <TimerOne.h>
//#########################
...
#define socketPin_B0 (13) // 3->13
#define socketPin_B1 (12) //4->12
#define socketPin_D0 (11) //5->11 //Socket pin w/ DIMMER
#define dimmerPin (6)//6->6
#define buttonPin_0 (8)//7->9//
#define buttonPin_1 (9)//8->8//
#define buttonPin_2 (10)//9-7//
#define buttonLine (7) //3->5
int currentPin_0 = A0;
int currentPin_1 = A1;
int currentPin_2 = A2;
////
...
////
//TESTING VARIABLES
volatile long ISRDuration = 0; //time (in microSeconds) it takes for ISR to execute
volatile long ISRMaxDuration = 0;
volatile long ISRAlfa = 0; // initial time (in microseconds) when ISR started to execute
String ISRMax = "ISR MAX: "; 
////
//POWER VARIABLES
...
////
//DIMMER VARIABLES
#define brightnessSteps (128); //An arbitrary number used to define certain number of brightness steps
#define networkFrequency (50);//Electric network frequency in [Hz]
#define TRIAC_OnTime (10);//Time it takes for triac state to change in [us]
volatile byte increment = 0;             // Variable to use as a dimmer counter
...
volatile byte brightness = 10 ; // Dimming level (0-128) 0 = on;128 = off
int freqStep = 75; // [us] ((freqStep = 1000000/2*networkFrequency)-TRIAC_OnTime)/brightnessSteps
////
...
////
struct buttonStruct{
	uint8_t cState : 1; //Current buttonState
	uint8_t pState : 1; //Last buttonState
	uint8_t buttPin : 4;
} button[3];
...
void dim_check() { //Timer interupput function called every 75uS or in variable terms every freqStep
	...
	ISRAlfa = micros(); // 	
	//#################Call this part only every 25ms
	/*
...
*/	
	//#########################
	if ((zero_cross == true) && (powerDimmer == true)) { //Cross turns of the the TRIAC if cross is present then dim_check turns
	...
	/// Dimmer block ENDS!	
	/////KEEP CODE BELOW WITH ANY ITTERATIONS ABOVE!
	ISRDuration = ISRAlfa-micros();
	if(ISRDuration>ISRMaxDuration)
	ISRMaxDuration = ISRDuration;
}
void setup()
{
...
void loop()
{
	//#######TESTING AREA
//Serial.print(ISRMax);
	//Serial.println(ISRMaxDuration);		


Serial.println(zero_cross);
//zero_cross = false;

	//#############################
	if(((millis()%1500)==0)&&(demo == true)){ //Initialize demo //SOMEHOW COUNTER GETS ALL THE VALUES INSTANTLY instead of 2 sec increase
	
delay(1);// ensures the loop going 1 time in 1ms
		if(demoCounter<3){
			ctrlSockets(demoCounter,ON);		
			demoCounter++;
		}//if1
		else{//more than 2
			demoCounter=0;
...
void ctrlSockets(byte socketID, byte socketState) { //Assuming sockets are counted from 0; socketID is the id of a specific socket; socketState is on or off or dim ; statesByte is the refere
	...
	case 0:
		...
		if(socketState == ON){
			bitWrite(stateByte, socketID,ON);
			digitalWrite(socketPin_B0,HIGH);
		}
		else if (socketState == OFF){
			bitWrite(stateByte, socketID,OFF);
